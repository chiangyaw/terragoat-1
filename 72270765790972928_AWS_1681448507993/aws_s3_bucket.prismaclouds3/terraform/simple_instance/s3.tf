...
resource "aws_s3_bucket" "prismaclouds3_log_bucket" {
  bucket = "prismaclouds3-log-bucket"
}

resource "aws_s3_bucket_logging" "prismaclouds3" {
  bucket = aws_s3_bucket.prismaclouds3.id

  target_bucket = aws_s3_bucket.prismaclouds3_log_bucket.id
  target_prefix = "log/"
}